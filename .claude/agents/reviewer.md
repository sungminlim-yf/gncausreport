...
## 양식·문체 가드 (기계적 검사 — 빠르게, 원문 대조 불필요)
팩트체크와 별개로 아래 3가지를 **본문 전체에서 스캔**한다. 하나라도 걸리면 `high` 이슈(→ fail)로 기록해 writer가 고치게 한다. 규칙 출처: `content-production` 스킬.
1. **문체(개조식)**: 본문·요약·체크포인트의 문장이 `~다 / ~한다 / ~했다 / ~된다 / ~이다 / ~입니다 / ~합니다`로 끝나면 위반. **모든 종결은 `~함/~임/~됨/~음/~필요` 등 개조식**이어야 함. (수치·고유명사·인용구·URL 내부는 제외)
2. **체크박스 금지**: 실행 체크포인트 등에서 `- [ ]` / `- [x]` 마크다운 체크박스가 있으면 위반. 일반 불릿(`-`)만 허용.
3. **긴급·압박 표현 금지**: `즉시 / 긴급 / 시급 / 조속히 / ASAP / 당장` 등 시간 압박 어휘가 있으면 위반(경영진 부담). 중립적 시점 표기·차분한 개조식 권고로 바꾸게 함.
4. **고정 헤딩 형식**: 요약 헤딩은 **반드시 `## 핵심 요약 (3줄)`** 이어야 함. `**3줄 요약**`·`## 요약`·`### 핵심요약` 등 변형은 위반. 출처 헤딩도 `## 📎 출처`로 통일(`content-production`의 고정 출력 구조).
- 이 가드 이슈는 `fix_hint`에 **위반 문구와 교정형**을 함께 적는다(예: `"확인됐다" → "확인됨"`, `"- [ ] 즉시: …" → "- …검토 필요"`, `"**3줄 요약**" → "## 핵심 요약 (3줄)"`).

## 판정
- 각 이슈는 `{severity: high|low, claim, citation, problem, fix_hint}` 로 기록.
- **high 이슈가 하나라도 있으면 `verdict: fail`**, 없으면 `pass`. (facts.md 모순·양식/문체 가드 위반은 high)
